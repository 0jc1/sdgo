...
def get_device():
    return open("backend/device.txt").readlines()[0]

def get_image_count_old():
    out_dir = os.path.abspath("./outputs")
    exts = ["*.png", "*.jpg", "*.webp"]
    exts = exts+[e.upper() for e in exts]
    images = []
    for ext in exts:
        images.extend(glob(os.path.join(out_dir, ext)))
...
def get_image_count():
    ...
    exts = exts+[e.upper() for e in exts]
    images = []
    for fname in glob(os.path.join(out_dir, "*.*")):
        if compiled.match(os.path.basename(fname)):
            images.append(fname)
    return len(set(images))
...
class SD3:
    def __init__(self, model, shift, verbose=False):
        with safe_open(model, framework="pt", device=get_device()) as f:
            ...
            load_into(f, self.model, "model.", get_device(), torch.float16)


class VAE:
    def __init__(self, model):
        with safe_open(model, framework="pt", device=get_device()) as f:
            self.model = SDVAE(device=get_device(), dtype=torch.float16).eval()
            prefix = ""
            if any(k.startswith("first_stage_model.") for k in f.keys()):
                prefix = "first_stage_model."
            load_into(f, self.model, prefix, get_device(), torch.float16)
...
MODEL = "backend/models/sd3.5_large.safetensors"
...
class SD3Inferencer:
    ...
    def load(self, model=MODEL, vae=VAEFile, shift=SHIFT, verbose=False):
        ...
        self.vae = VAE(model)#(vae or model)
        print("Models loaded.")
    ...
    def do_sampling(
        ...
        sample_fn = getattr(sd3_impls, f"sample_{sampler}_preview")
        denoiser = (
            SkipLayerCFGDenoiser
        ...
        latent = sample_fn(
            ...
            sigmas,
            SD3LatentFormat(),
            "./outputs/prev-"+os.path.basename(fname),
    ...
    def gen_image(
        ...
        neg_cond = self.get_cond(neg_prompt)
        seed_num = None
        ...
        for i, prompt in pbar:
            ...
            #save_path = os.path.join(out_dir, f"{get_image_count():06d}.png")
            save_path = os.path.join(out_dir, selected_name)
...
def backend_proc(q_in, q_out):
    content = q_in.get()
    model = content.get("model")
    vae = content.get("vae")
    verbose = True
    skip_layer_cfg = False
    steps = content.get("steps") or CONFIGS.get(os.path.splitext(os.path.basename(model))[0], {}).get(
        "steps", 50
    )
    cfg = content.get("cfg") or CONFIGS.get(os.path.splitext(os.path.basename(model))[0], {}).get(
        "cfg", 5
    )
    shift = content.get("shift") or CONFIGS.get(os.path.splitext(os.path.basename(model))[0], {}).get(
        "shift", 3
    )
    sampler = content.get("sampler") or CONFIGS.get(
        os.path.splitext(os.path.basename(model))[0], {}
    ).get("sampler", "dpmpp_2m")
    if skip_layer_cfg:
        skip_layer_config = CONFIGS.get(
...
if __name__=="__main__":
    ...
    queue = Queue()
    queue_out = Queue()
    proc = Process(target = backend_proc, args = (queue, queue_out))
    started = False
    ...
    def start_server():
        ...
        proc.start()
        queue.put(content)
        return "<p>The server has started!</p>"
    ...
    def gen_status():
        content = request.json
        preview = "./outputs/prev-"+os.path.basename(content.get("name"))
        fname = "./outputs/"+os.path.basename(content.get("name"))
        print("Fname:", fname, "Preview:", preview)
        d = {"status": (0 if not os.path.exists(preview) else 1),
             "path": ("" if not os.path.exists(preview) else os.path.abspath(preview))
             }
        if os.path.exists(fname) and not os.path.basename(fname).startswith("prev"):
            d["status"] = 2
            d["path"] = os.path.abspath(fname)
            os.remove(preview)
        return jsonify(d)

    @app.route("/end")
    def exiter():
        global proc, queue, queue_out
